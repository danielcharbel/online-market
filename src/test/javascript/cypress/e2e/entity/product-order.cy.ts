...
describe('ProductOrder e2e test', () => {
  ...
  // const productOrderSample = {"quantity":2949,"totalPrice":31752.14};
  ...
  beforeEach(() => {
    ...
    cy.authenticatedRequest({
      method: 'POST',
      url: '/api/products',
      body: {"name":"hence","description":"legislate vie","price":25084.7,"productSize":"XXL","image":"Li4vZmFrZS1kYXRhL2Jsb2IvaGlwc3Rlci5wbmc=","imageContentType":"unknown"},
    }).then(({ body }) => {
      product = body;
    });
    // create an instance at the required relationship entity:
    cy.authenticatedRequest({
      ...
      body: {"placedDate":"2024-01-13T13:47:36.499Z","status":"CANCELLED","totalPrice":10488.39,"paymentMethod":"IDEAL"},
    }).then(({ body }) => {
      shoppingCart = body;
    });
  });
  ...
  describe('new ProductOrder page', () => {
    ...
    it.skip('should create an instance of ProductOrder', () => {
      cy.get(`[data-cy="quantity"]`).type('26934');
      cy.get(`[data-cy="quantity"]`).should('have.value', '26934');

      cy.get(`[data-cy="totalPrice"]`).type('29410.25');
      cy.get(`[data-cy="totalPrice"]`).should('have.value', '29410.25');
