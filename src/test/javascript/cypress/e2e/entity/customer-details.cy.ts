...
describe('CustomerDetails e2e test', () => {
  ...
  // const customerDetailsSample = {"gender":"FEMALE","phone":"547.683.6801 x082","addressLine1":"across apud beast","city":"Fort Martine","country":"Georgia"};
  ...
  beforeEach(() => {
    ...
    cy.authenticatedRequest({
      method: 'POST',
      url: '/api/users',
      body: {"login":"above uh-huh um","firstName":"Madison","lastName":"Altenwerth"},
    }).then(({ body }) => {
      user = body;
    });
  });
  ...
  describe('new CustomerDetails page', () => {
    ...
    it.skip('should create an instance of CustomerDetails', () => {
      ...
      cy.get(`[data-cy="phone"]`).type('(489) 864-1137 x227');
      cy.get(`[data-cy="phone"]`).should('have.value', '(489) 864-1137 x227');

      cy.get(`[data-cy="addressLine1"]`).type('fortunately boo');
      cy.get(`[data-cy="addressLine1"]`).should('have.value', 'fortunately boo');

      cy.get(`[data-cy="addressLine2"]`).type('key ew inconsequential');
      cy.get(`[data-cy="addressLine2"]`).should('have.value', 'key ew inconsequential');

      cy.get(`[data-cy="city"]`).type('Lake Bonniebury');
      cy.get(`[data-cy="city"]`).should('have.value', 'Lake Bonniebury');

      cy.get(`[data-cy="country"]`).type('Belarus');
      cy.get(`[data-cy="country"]`).should('have.value', 'Belarus');
